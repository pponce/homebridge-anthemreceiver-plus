...
exports.strListeningModes = void 0;
exports.isSTR = isSTR;
exports.isSTRCommand = isSTRCommand;
...
function isSTRCommand(command) {
    if (/^(IDM|IDS|IDN|ICN|ISN\d{2}|Z1(?:POW|MUT|VOL|INP|ALM))\?$/.test(command))
        return true;
    if (/^Z1(?:POW|MUT)[01]$/.test(command))
        return true;
    if (/^Z1ALM(?:7|9|11|12)$/.test(command))
        return true;
    const volume = /^Z1VOL([+-]?\d+(?:\.\d+)?)$/.exec(command);
    if (volume) {
        const db = Number(volume[1]);
        return db >= -96 && db <= 7 && Number.isInteger(db * 2);
    }
...
function capabilities(model) {
    ...
    if (isSTR(normalized))
        return { model: normalized, ...strProfiles[normalized],
    ...
    if (!older.includes(normalized) && !newer.includes(normalized))
        throw new Error(`Unsupported Anthem model: ${normalized.slice(0, 80)}`);
